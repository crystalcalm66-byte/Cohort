...
const SUBJECTS: Array<{
  ...
  // A Level (same subjects, more advanced topics)
  ...
  { name: "Reading & Writing", exam_track: "sat", icon: "📚", color: "#6366F1", order_index: 0,
    units: [
      ...
      { name: "Standard English Conventions", order_index: 3, topics: ["Subject-Verb Agreement", "Pronoun Agreement", "Verb Tenses", "Modifier Placement", "Parallel Structure", "Punctuation (Commas, Semicolons, Colons, Dashes)"] },
    ] },
  { name: "Math", exam_track: "sat", icon: "📐", color: "#0D9488", order_index: 1,
    units: [
      ...
      { name: "Problem Solving & Data Analysis", order_index: 2, topics: ["Percentages & Ratios", "Unit Conversion", "Statistics (Mean, Median, SD)", "Probability", "Sampling & Surveys"] },
...
async function seed() {
  ...
  for (const subject of SUBJECTS) {
    ...
    if (!subj) { console.error(`Failed to insert subject: ${subject.name}`); continue }
    ...
    for (const unit of subject.units) {
      const { data: unitData } = await supabase
        .from("units")
        .insert({ subject_id: subj.id, name: unit.name, order_index: unit.order_index })
        .select()
        .single()

      if (!unitData) { console.error(`    Failed to insert unit: ${unit.name}`); continue }

      for (let i = 0; i < unit.topics.length; i++) {
        await supabase
          .from("topics")
          .insert({ unit_id: unitData.id, name: unit.topics[i], order_index: i })
      }
    }
  }
